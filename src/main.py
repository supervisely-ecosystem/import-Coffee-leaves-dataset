...
def create_ann(ann_path):
    ...
    return sly.Annotation(img_size=(ann_np_leaf.shape[0], ann_np_leaf.shape[1]), labels=labels, img_tags=tags)
...
def import_strawberry(api: sly.Api, task_id, context, state, app_logger):
    ...
    coffee_data_path = os.path.join(g.work_dir_path, sly.io.fs.get_file_name(g.arch_name))
    ...
    for ds in g.datasets:
        ...
        for img_batch in sly.batched(sample_img_path, batch_size=g.batch_size):
            ...
            ann_pathes = [os.path.join(curr_ann_path, get_file_name(name) + g.ann_ext) for name in img_batch]
